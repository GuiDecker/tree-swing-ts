...
const syncSingleChild = async (
  ...
  try {
    ...
    process.stdout.write(
      renderColor(
        `⚠️  Failed to switch to "${childBranch}". Skipping.\n`,
        COLORS.RED,
      ),
    );
    return false;
  ...
  try {
    ...
    process.stdout.write(
      renderColor(
        `⚠️  Merge conflicts detected on "${childBranch}".\n`,
        COLORS.RED,
      ),
    );
    process.stdout.write(
      renderColor(
        `   Please resolve them manually before syncing the remaining branches.\n`,
        COLORS.RED,
      ),
    );
    return false;
  ...
  const answer = await makeQuestion(
    renderColor(`Push "${childBranch}" to origin now? (y/n)`, COLORS.CYAN),
  );
  ...
  if (answer.trim().toLowerCase() === "y") {
    try {
      ...
      process.stdout.write(
        renderColor(`✅ Pushed "${childBranch}".\n`, [
          COLORS.BOLD,
          COLORS.GREEN,
        ]),
      );
      ...
      process.stdout.write(
        renderColor(`⚠️  Failed to push "${childBranch}".\n`, COLORS.RED),
      );
    }
  }
...
const runSync = async (
  ...
  for (const childBranch of childBranches) {
    const ok = await syncSingleChild(parentBranch, childBranch);
...
const resolveChildren = async () => {
  ...
  const usesPrefix = configs.some((config) =>
    parentBranch.startsWith(`${config.value}/`),
  );

  if (usesPrefix) {
    process.stdout.write(
      renderColor(
        `⚠️  You are on a child branch ("${parentBranch}").\n`,
        COLORS.YELLOW,
      ),
    );
    process.stdout.write(
      renderColor(
        `   Switch to the parent feature branch before syncing.\n`,
        COLORS.RED,
      ),
    );
    showCursor();
...
export const showSyncBranches = async () => {
  ...
  process.stdout.write(
    renderColor("⏳ Looking for child branches...\n", COLORS.CYAN),
  );
  ...
  if (children.length === 0) {
    process.stdout.write(
      renderColor(
        `⚠️  No child branches found for "${parentBranch}".\n`,
        COLORS.YELLOW,
      ),
    );
  ...
  const menuOptions = [
    {
      id: 1,
      label: `Sync all child branches (${children.length})`,
      value: "all",
      action: () => runSync(parentBranch, children.map((c) => c.branch)),
    },
    ...children.map((child, idx) => ({
      id: idx + 2,
      label: `Sync only ${child.branch}`,
      value: child.branch,
      action: () => runSync(parentBranch, [child.branch]),
    })),
    {
  ...
  const title = renderColor(
    `🔄 Sync child branches of "${parentBranch}"`,
